...
let announcements = JSON.parse(localStorage.getItem('frh_announcements')) || [
    { id: 1, title: "Selamat Datang di FileHub Ultimate Suite v8!", content: "Fitur link iklan & link tanpa iklan (Rp 5.000/bulan) serta kontrol izin user admin aktif.", date: "10 Agustus 2026" }
];
...
let liveChatMessages = JSON.parse(localStorage.getItem('frh_livechat_msgs')) || [
    { sender: "System", text: "Selamat datang di Live Chat Pembayaran Langganan (Rp 5.000/bulan) untuk akses Link Tanpa Iklan sepuasnya atau pembelian akses post satuan!", time: "Baru saja" }
];
...
let userVipSubscriptions = JSON.parse(localStorage.getItem('frh_user_vip_subs')) || {}; // username: true/false
...
let notifications = JSON.parse(localStorage.getItem('frh_notifications')) || [
    { id: 1, text: "Selamat datang di platform FileHub Ultimate Suite v8!", type: 'info', read: false, time: "Baru saja" }
];
...
function autoExtractChangelog() {
    const desc = document.getElementById('up-desc').value;
    if (!desc) {
        alert('Isi deskripsi terlebih dahulu sebelum melakukan ekstraksi otomatis.');
        return;
    }
    if (!desc.includes('[Changelog')) {
        document.getElementById('up-desc').value = desc + "\n\n[Changelog v1.0]: Rilis awal pembaruan stabil.";
        alert('Format catatan rilis otomatis berhasil ditambahkan ke deskripsi.');
    } else {
        alert('Catatan rilis sudah terdeteksi di dalam deskripsi.');
    }
}
...
function switchAdminTab(type) {
    ['upload', 'manage', 'users', 'requests', 'analytics', 'telegram', 'backup', 'settings'].forEach(t => {
        ...
        list.innerHTML += `
            <div class="bg-slate-950 p-4 rounded-xl border border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-3 text-xs">
                <div>
                    ...
                    <span class="text-[10px] text-slate-400">Status: ${u.banned ? '<span class="text-rose-400">Diblokir</span>' : '<span class="text-emerald-400">Aktif</span>'} | VIP Tanpa Iklan (Rp 5rb/bln): <span class="${isVip ? 'text-amber-400 font-bold' : 'text-slate-400'}">${isVip ? 'Aktif (VIP)' : 'Non-VIP'}</span></span>
                </div>
                <div class="flex gap-2">
                    <button onclick="toggleVipSubscription('${u.username}')" class="px-3 py-1.5 ${isVip ? 'bg-amber-500 text-slate-950' : 'bg-slate-800 text-amber-400'} font-bold rounded-lg cursor-pointer">${isVip ? 'Cabut VIP Tanpa Iklan' : 'Beri VIP Tanpa Iklan'}</button>
...
function handleSendLiveChat(e) {
    e.preventDefault();
    const input = document.getElementById('livechat-input');
    const text = input.value.trim();
    if (!text) return;
    liveChatMessages.push({ sender: currentUser.username, text, time: new Date().toLocaleTimeString('id-ID') });
    localStorage.setItem('frh_livechat_msgs', JSON.stringify(liveChatMessages));
    input.value = '';
    renderLiveChatMessages();
}

function renderLiveChatMessages() {
    const box = document.getElementById('livechat-messages');
    box.innerHTML = '';
    liveChatMessages.forEach(msg => {
        let isMe = currentUser && msg.sender === currentUser.username;
        box.innerHTML += `
            <div class="p-3 rounded-xl bg-slate-950 border border-slate-800 space-y-1 ${isMe ? 'border-cyan-500/30' : ''}">
                <div class="flex justify-between text-[10px] text-slate-400">
                    <span class="font-bold text-cyan-400">${msg.sender}</span>
                    <span>${msg.time}</span>
                </div>
                <p class="text-slate-200">${msg.text}</p>
                ${currentUser && currentUser.role === 'admin' && msg.sender !== 'System' ? `
                    <div class="flex gap-2 pt-1">
                        <button onclick="approveVipChat('${msg.sender}')" class="px-2.5 py-1 bg-amber-500 text-slate-950 font-bold rounded text-[10px] cursor-pointer">Setujui VIP Tanpa Iklan (Rp 5k/bln)</button>
                    </div>
                ` : ''}
            </div>
        `;
...
function approveVipChat(targetUser) {
    userVipSubscriptions[targetUser] = true;
    localStorage.setItem('frh_user_vip_subs', JSON.stringify(userVipSubscriptions));
    liveChatMessages.push({ sender: "System", text: `Langganan Link Tanpa Iklan (Rp 5.000/bulan) untuk @${targetUser} telah disetujui admin! Akses VIP diaktifkan.`, time: new Date().toLocaleTimeString('id-ID') });
    localStorage.setItem('frh_livechat_msgs', JSON.stringify(liveChatMessages));
    renderLiveChatMessages();
    alert(`Langganan VIP Tanpa Iklan untuk ${targetUser} berhasil disetujui.`);
}
...
function saveTelegramSettings(e) {
    ...
    localStorage.setItem('frh_tg_config', JSON.stringify(telegramConfig));
...
function handleSaveResource(e) {
    ...
    if (editId) {
        ...
        alert('Tautan resource & opsi iklan berhasil diperbarui!');
        resetUploadForm();
    } else {
        const newRes = {
            id: Date.now(),
            name, category, subcategory, version,
            linkAd, linkNoAd, price, paidUnlockedUsers: [],
...
function editResource(id) {
    ...
    document.getElementById('up-price').value = res.price || 0;
        ...
        list.innerHTML += `
            <div class="bg-slate-950 border border-slate-800 p-4 rounded-xl flex flex-col justify-between text-xs space-y-3">
                <div>
                    ...
                    <div class="text-emerald-400 mt-1"><i class="fa-solid fa-link"></i> Link Iklan & Tanpa Iklan Aktif</div>
                </div>
...
function exportDataBackup() {
    ...
    dlAnchor.setAttribute("download", "filehub_ultimatesuite_v8_backup.json");
...
function populateSubcategoriesDropdown() {
    const select = document.getElementById('subcat-filter');
    if (!select) return;
    let currentVal = select.value;
    select.innerHTML = `<option value="All">Semua Sub Kategori</option>`;
    
    let subcategories = [...new Set(resources.map(r => r.subcategory))];
...
function renderResources() {
    ...
    const subcatFilter = document.getElementById('subcat-filter').value;
    grid.innerHTML = '';
    ...
    let filtered = resources.filter(res => {
        ...
        if (subcatFilter !== 'All') {
            matchSubcat = res.subcategory === subcatFilter;
        }
                ...
                <div>
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[10px] font-bold px-2 py-0.5 rounded bg-amber-500/20 text-amber-400 border border-amber-500/30"><i class="fa-solid fa-shield-halved"></i> Iklan & Tanpa Iklan</span>
                        </div>
                        <div class="flex items-center gap-2">
                            ${isCommunityChoice ? '<span class="text-[10px] text-amber-300 font-bold bg-amber-500/20 px-2 py-0.5 rounded border border-amber-500/30"><i class="fa-solid fa-fire"></i> Favorit</span>' : ''}
                            ${isBroken ? '<span class="text-[10px] text-rose-400 font-bold bg-rose-500/10 px-2 py-0.5 rounded"><i class="fa-solid fa-triangle-exclamation"></i> Rusak</span>' : ''}
...
function openDetail(id, openModalWindow = true) {
    ...
    let isUnlockedPost = res.paidUnlockedUsers && res.paidUnlockedUsers.includes(currentUser.username);

    if (currentUser.role === 'admin' || isVip || isUnlockedPost) {
        downloadNoAdBtn.href = res.linkNoAd;
        downloadNoAdBtn.innerHTML = `<i class="fa-solid fa-shield-halved"></i> Link Tanpa Iklan (${res.fileSize}) [Akses VIP Aktif]`;
    } else {
        downloadNoAdBtn.href = "#";
        downloadNoAdBtn.onclick = (e) => {
            e.preventDefault();
            alert('Link Tanpa Iklan memerlukan langganan VIP (Rp 5.000/bulan) atau pembelian akses post ini. Silakan konfirmasi pembayaran melalui Live Chat.');
            switchMainView('livechat');
            closeModal();
        };
        downloadNoAdBtn.innerHTML = `<i class="fa-solid fa-lock"></i> Link Tanpa Iklan (Terkunci - Langganan 5k/bln)`;
    }
...
function rateResource(star) {
    ...
    logUserAction(currentUser.username, `Memberi rating ${star} bintang pada ${res.name}`);
    localStorage.setItem('frh_resources', JSON.stringify(resources));
    alert(`Terima kasih! Anda memberikan rating ${star} bintang dan mendapatkan +10 Poin.`);
...
function recordDownload(e, type) {
    ...
    if (type === 'noad') {
        let res = resources.find(r => r.id === activeResourceId);
        let isVip = userVipSubscriptions[currentUser.username] || false;
        let isUnlocked = res.paidUnlockedUsers && res.paidUnlockedUsers.includes(currentUser.username);
        if (currentUser.role !== 'admin' && !isVip && !isUnlocked) {
            e.preventDefault();
            alert('Akses Link Tanpa Iklan terkunci. Silakan berlangganan VIP (Rp 5.000/bulan) via Live Chat.');
            switchMainView('livechat');
            closeModal();
            return;
        }
...
function renderProfilePage() {
    ...
    let isVip = userVipSubscriptions[currentUser.username] || false;
    document.getElementById('profile-vip-status').innerHTML = isVip ? `<span class="px-2.5 py-1 bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded-lg"><i class="fa-solid fa-shield-halved"></i> VIP Tanpa Iklan Aktif (Rp 5.000/bln)</span>` : `<span class="text-slate-400">Status: Member Free (Dengan Iklan)</span>`;

    const trophyBox = document.getElementById('profile-trophies');
    trophyBox.innerHTML = '';
    let pts = userPoints[currentUser.username] || 0;
    if (pts >= 20) trophyBox.innerHTML += `<span class="px-2 py-0.5 bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded-full text-[10px] font-bold"><i class="fa-solid fa-medal"></i> Active Reviewer</span>`;
    if (pts >= 50) trophyBox.innerHTML += `<span class="px-2 py-0.5 bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 rounded-full text-[10px] font-bold"><i class="fa-solid fa-shield-halved"></i> Bug Hunter</span>`;
    ...
    const auditList = document.getElementById('profile-audit-list');
    auditList.innerHTML = '';
    let myAudits = userAuditLogs[currentUser.username] || [];
    if (myAudits.length === 0) {
        auditList.innerHTML = `<p class="text-xs text-slate-500">Belum ada aktivitas tercatat.</p>`;
    } else {
        myAudits.forEach(aud => {
            auditList.innerHTML += `<div class="bg-slate-950 p-2.5 rounded-xl text-[11px] text-slate-300 border border-slate-800 flex justify-between"><span>${aud.text}</span><span class="text-slate-500">${aud.time}</span></div>`;
        });
    }
}
