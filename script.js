...
let resources = JSON.parse(localStorage.getItem('frh_resources')) || [
    ...
    },
    {
        ...
        version: "v2.0",
        tags: ["offline", "no-root"],
        description: "Kumpulan aset template ZIP HTML, CSS, dan Tailwind.\n\n[Changelog v2.0]: Penambahan mode gelap dan komponen tabel dinamis.",
...
let announcements = JSON.parse(localStorage.getItem('frh_announcements')) || [
    { id: 1, title: "Selamat Datang di FileHub Ultimate Suite v5!", content: "10 Fitur baru & 10 penyesuaian lanjutan telah berhasil diintegrasikan.", date: "10 Agustus 2026" }
];
...
let notifications = JSON.parse(localStorage.getItem('frh_notifications')) || [
    { id: 1, text: "Selamat datang di platform FileHub Ultimate Suite v5!", type: 'info', read: false, time: "Baru saja" }
];
...
function switchMainView(view) {
    ['user-panel', 'profile-panel', 'faq-panel', 'leaderboard-panel', 'requests-panel'].forEach(id => {
...
function switchAdminTab(type) {
    ['upload', 'announcement', 'manage', 'requests', 'analytics', 'telegram', 'backup', 'settings'].forEach(t => {
...
function handleSaveResource(e) {
    ...
    if (editId) {
        ...
        const newRes = {
            ...
            tags: ["root", "offline"],
            description, fileData, fileSize, screenshot, verified,
...
function exportDataBackup() {
    ...
    dlAnchor.setAttribute("download", "filehub_ultimatesuite_v5_backup.json");
...
function renderResources() {
    ...
    let filtered = resources.filter(res => {
        ...
        let matchTag = true;
        if (tagFilter !== 'All') {
            matchTag = res.tags && res.tags.includes(tagFilter);
        }
        ...
        return matchCat && matchFormat && matchTag && matchSearch;
    });
        ...
        grid.innerHTML += `
            <div class="bg-slate-900 border ${isBroken ? 'border-rose-500/50' : 'border-slate-800/80'} rounded-2xl p-5 flex flex-col justify-between hover:border-slate-700 transition-all shadow-lg group">
                <div>
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-2">
                            ...
                            ${res.verified ? '<span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-400" title="Aman & Teruji"><i class="fa-solid fa-shield-check"></i></span>' : ''}
                        </div>
                        <div class="flex items-center gap-2">
                ...
                <div class="pt-4 mt-4 border-t border-slate-800/60 flex items-center justify-between text-xs text-slate-300">
                    ...
                    <div class="flex items-center gap-2">
                        <button onclick="copyDirectLink(${res.id})" class="p-2 rounded-lg bg-slate-800 hover:bg-slate-700 transition-colors cursor-pointer text-cyan-400" title="Salin Tautan Cepat">
                            <i class="fa-solid fa-copy"></i>
                        </button>
                        <button onclick="promptSaveToFolder(${res.id})" class="p-2 rounded-lg bg-slate-800 hover:bg-slate-700 transition-colors cursor-pointer ${isSaved ? 'text-amber-400' : 'text-slate-300'}" title="Simpan ke Folder">
...
function promptSaveToFolder(id) {
    activeResourceId = id;
    const modal = document.getElementById('save-to-folder-modal');
    const list = document.getElementById('folder-selection-list');
    list.innerHTML = '';
    
    let cols = userCollections[currentUser.username] || {};
    list.innerHTML += `<button onclick="toggleSave(${id})" class="w-full text-left px-3 py-2 bg-slate-950 hover:bg-slate-800 rounded-xl text-xs text-cyan-400">Simpan Utama (Default)</button>`;
    
    for (let cName in cols) {
        list.innerHTML += `<button onclick="saveToCustomFolder('${cName}', ${id})" class="w-full text-left px-3 py-2 bg-slate-950 hover:bg-slate-800 rounded-xl text-xs text-amber-400">📁 Folder: ${cName}</button>`;
    }
    modal.classList.remove('hidden');
}

function closeFolderModal() {
    document.getElementById('save-to-folder-modal').classList.add('hidden');
}

function saveToCustomFolder(folderName, id) {
    let res = resources.find(r => r.id === id);
    if (!userCollections[currentUser.username][folderName].includes(res.name)) {
        userCollections[currentUser.username][folderName].push(res.name);
        localStorage.setItem('frh_user_collections', JSON.stringify(userCollections));
        alert(`Berhasil disimpan ke folder "${folderName}"!`);
    } else {
        alert('File sudah ada di dalam folder ini.');
    }
    closeFolderModal();
}
...
function copyDirectLink(id) {
    const res = resources.find(r => r.id === id);
    if (!res) return;
    navigator.clipboard.writeText(res.fileData);
...
function recordDownload() {
    ...
    logUserAction(currentUser.username, `Mengunduh file: ${res.name}`);
...
function renderProfilePage() {
    ...
    if (pts >= 100) trophyBox.innerHTML += `<span class="px-2 py-0.5 bg-purple-500/20 text-purple-400 border border-purple-500/30 rounded-full text-[10px] font-bold"><i class="fa-solid fa-crown"></i> Master Contributor</span>`;
    ...
    let cols = userCollections[currentUser.username] || {};
    for (let cName in cols) {
        savedList.innerHTML += `<div class="p-3 bg-slate-950 border border-amber-500/30 rounded-xl text-xs mt-2 space-y-1"><span class="font-bold text-amber-400 block"><i class="fa-solid fa-folder"></i> Folder: ${cName}</span><span class="text-[10px] text-slate-400">${cols[cName].length} item tersimpan</span></div>`;
    }
