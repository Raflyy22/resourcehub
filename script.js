...
let adminCreds = JSON.parse(localStorage.getItem('frh_admin_creds')) || { user: 'admin', pass: 'admin123', pin: '9999' };
...
let resources = JSON.parse(localStorage.getItem('frh_resources')) || [
    {
        ...
        category: "Aplikasi",
        subcategory: "Modifikasi Builder",
...
let announcements = JSON.parse(localStorage.getItem('frh_announcements')) || [
    { id: 1, title: "Selamat Datang di File Resource Hub Pro!", content: "Semua file kini tersedia via tautan unduhan eksternal.", date: "10 Agustus 2026" }
];
...
document.addEventListener('DOMContentLoaded', () => {
    checkAuthState();
    document.getElementById('set-admin-user').value = adminCreds.user;
    document.getElementById('set-admin-pass').value = adminCreds.pass;
    document.getElementById('set-admin-pin').value = adminCreds.pin;
});
...
function handleLogin(e, role) {
    ...
    if (role === 'admin') {
        ...
        if (user === adminCreds.user && pass === adminCreds.pass && pin === adminCreds.pin) {
            currentUser = { username: 'Administrator', role: 'admin' };
        } else {
            alert('Username, Password, atau PIN Admin salah!');
            return;
        }
    } else {
...
function switchAdminTab(type) {
    ['upload', 'announcement', 'manage', 'analytics', 'settings'].forEach(t => {
...
function renderAdminAnalytics() {
    ...
    brokenReports.forEach((rep, idx) => {
        repList.innerHTML += `
            <div class="bg-slate-950 border border-slate-800 p-3 rounded-xl text-xs flex justify-between items-center">
                <div>
                    <span class="font-bold text-rose-400">${rep.resName}</span> dilaporkan rusak oleh <span class="text-cyan-400">${rep.user}</span>
                </div>
                <button onclick="resolveReport(${idx})" class="px-3 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg cursor-pointer">Tandai Selesai</button>
            </div>
        `;
    });
}
...
function reportBrokenLink() {
    ...
    localStorage.setItem('frh_broken_reports', JSON.stringify(brokenReports));
    alert('Laporan link rusak telah dikirim ke Administrator.');
}
