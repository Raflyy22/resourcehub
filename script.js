...
let announcements = JSON.parse(localStorage.getItem('frh_announcements')) || [
    { id: 1, title: "Selamat Datang di FileHub Ultimate Suite v9!", content: "Live chat real-time dengan kirim foto, redeem poin reward, dan kontrol user admin kini aktif.", date: "10 Agustus 2026" }
];
...
// Live Chat Data: { username: [ { sender, text, img, time } ] }
...
function switchAdminTab(type) {
    ['upload', 'manage', 'users', 'livechat', 'rewards', 'requests', 'analytics', 'telegram', 'backup', 'settings'].forEach(t => {
...
// Kontrol User & Akses Postingan di Dashboard Admin
            ...
            <div class="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3 text-xs">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                    <div>
                        ...
                        <span class="text-[10px] text-slate-400">Status: ${u.banned ? '<span class="text-rose-400">Diblokir</span>' : '<span class="text-emerald-400">Aktif</span>'} | VIP Tanpa Iklan: <span class="${isVip ? 'text-amber-400 font-bold' : 'text-slate-400'}">${isVip ? 'Aktif' : 'Non-VIP'}</span></span>
                    </div>
                    <div class="flex gap-2">
                        <button onclick="toggleVipSubscription('${u.username}')" class="px-3 py-1.5 ${isVip ? 'bg-amber-500 text-slate-950' : 'bg-slate-800 text-amber-400'} font-bold rounded-lg cursor-pointer">${isVip ? 'Cabut VIP' : 'Beri VIP Tanpa Iklan'}</button>
...
// Live Chat Real-Time Admin & User dengan Kirim Gambar/Foto
